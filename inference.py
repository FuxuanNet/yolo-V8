...
def calculate_map(predictions_all, ground_truth_all, iou_thresholds=np.arange(0.5, 1.0, 0.05)):
    """计算mAP@0.5:0.95"""
    aps = []
...
def measure_inference_speed(model, test_images, tile_size=640, overlap=128, iterations=10):
    """测量推理速度(FPS)"""
    total_time = 0
    total_frames = 0
...
def evaluate_model():
    ...
    for original_name in tqdm(original_val_images, desc="评估进度"):
        ...
        if len(merged_boxes) > 0 and len(ground_truth) > 0:
            ...
            precision, recall, f1_score, ap = calculate_precision_recall(pred_data, ground_truth)
    ...
    if len(predictions_all) > 0 and len(ground_truth_all) > 0:
        # 计算mAP@0.5
        map_05, _ = calculate_map(predictions_all, ground_truth_all, iou_thresholds=[0.5])
