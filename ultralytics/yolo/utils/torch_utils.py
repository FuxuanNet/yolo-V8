...
def model_info(model, verbose=False, imgsz=640):
    ...
    LOGGER.info(
        f"{m} summary: {len(list(model.modules()))} layers, {n_p} parameters, {n_g} gradients{fs}"
    )
...
def strip_optimizer(f="best.pt", s=""):
    ...
    x["train_args"] = {
        k: v for k, v in args.items() if k in DEFAULT_CONFIG_KEYS
...
def guess_task_from_head(head):
    ...
    if not task:
        raise SyntaxError(
            "task or model not recognized! Please refer the docs at : "
        )  # TODO: add docs links
